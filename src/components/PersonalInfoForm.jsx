        ...
        {typeof data.image === 'string' && (
            <div className='flex flex-col gap-1 pl-4 text-sm'>
                ...
                <label className='realtive inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                    <input type="checkbox" name="" id="" className='sr-only' checked={removeBackground} onChange={(e)=>setRemoveBackground(prev=> !prev)}/>
                <div className='w-9 h-5 bg-slate-300 rounded-full peer peer-checked:bg-indigo-600 transition-colors duration-200'>
                </div>
                <span className='dot absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition-transform duration-200 ease-in-out peer-checked:transition-transform peer-checked:translate-x-4'>
